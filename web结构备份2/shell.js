; (function () {
	...
	var enterLoadPath;
	var uiConfigName = "js/default.thm.min.js";
	var platformId;
	var configId = "13";
	...
	function $loadGameCfg() {
		...
		xhr.open('GET', './gameConfig.json?v=' + Math.random(), true);
	...
	function loadVerCfg(){
		...
		xhr.addEventListener("load", function ()
		{
			...
			enterLoadPath = "ver/" + defaultVer + "/" +uiConfigName;
			...
			for (var i = 0; i < ln; i++)
			{
				...
				if(jsFilePath.indexOf(uiConfigName) != -1){
					enterLoadPath = "ver/" + jsVer + "/" +uiConfigName;
				} else {
			...
			$nextImport();
			enterLoadPath = "ver/" + 19 + "/" +uiConfigName;
		});
		xhr.send(null);
	};
